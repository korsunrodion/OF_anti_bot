...
def run_test(n_holdout: int = 17, model_dir: str = 'models_cpu',
    ...
    actual_holdout = min(n_holdout, len(eligible))
    if actual_holdout < n_holdout:
        print(f"  Warning: only {len(eligible)} eligible models; "
              f"using {actual_holdout} holdout models", flush=True)

    holdout_models = set(eligible.head(actual_holdout).index.tolist())
    ...
    print(f"  Cold : {len(cold_subs)} rows in {actual_holdout} holdout models", flush=True)
